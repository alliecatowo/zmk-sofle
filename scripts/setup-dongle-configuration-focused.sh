...
# ZMK AllieCat Sofle Dongle Setup - Focused Version
...
create_backup() {
    ...
    [ -f "$PROJECT_ROOT/config/alliecat_keeb.keymap" ] && cp "$PROJECT_ROOT/config/alliecat_keeb.keymap" "$BACKUP_DIR/"
    [ -f "$PROJECT_ROOT/config/alliecat_keeb.conf" ] && cp "$PROJECT_ROOT/config/alliecat_keeb.conf" "$BACKUP_DIR/"
...
create_dongle_keymap() {
    ...
    local source_keymap="$PROJECT_ROOT/config/alliecat_keeb.keymap"
...
# ZMK AllieCat Sofle Dongle Quick Setup
...
- `alliecat_keeb_standard_left.uf2` - Left half
- `alliecat_keeb_standard_right.uf2` - Right half
- `alliecat_keeb_dongle_central.uf2` - Dongle (if needed)
- `settings_reset_*.uf2` - Reset utilities

### Flashing Process

#### Option A: Update Keyboard Halves Only
1. **Left half**: Double-tap reset → Copy `alliecat_keeb_standard_left.uf2`
2. **Right half**: Double-tap reset → Copy `alliecat_keeb_standard_right.uf2`
3. **Test connection**
        ...
        if west build --pristine; then
            ...
            case $choice in
                [Aa])
                    ...
                    echo "1. Left half: Double-tap reset → Copy alliecat_keeb_standard_left.uf2"
                    echo "2. Right half: Double-tap reset → Copy alliecat_keeb_standard_right.uf2"
                ...
                [Bb])
                    ...
                    echo "3. Flash alliecat_keeb_standard_left.uf2 to left"
                    echo "4. Flash alliecat_keeb_standard_right.uf2 to right"
...
# Main execution
main() {
    echo "🎮 ZMK AllieCat Sofle Dongle Setup (Focused)"
    echo "==========================================="
    echo ""
