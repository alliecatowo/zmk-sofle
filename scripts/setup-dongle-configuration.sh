...
# ZMK AllieCat Sofle Dongle Configuration Setup
...
check_prerequisites() {
    ...
    if [ ! -f "$PROJECT_ROOT/build.yaml" ]; then
        log_error "Please run this script from the ZMK AllieCat Sofle repository root"
        exit 1
    fi
...
backup_current_config() {
    ...
    if [ -f "$PROJECT_ROOT/config/alliecat_keeb.keymap" ]; then
        cp "$PROJECT_ROOT/config/alliecat_keeb.keymap" "$BACKUP_DIR/"
        log_success "Keymap backed up"
    fi

    if [ -f "$PROJECT_ROOT/config/alliecat_keeb.conf" ]; then
        cp "$PROJECT_ROOT/config/alliecat_keeb.conf" "$BACKUP_DIR/"
...
include:
  ...
  - board: alliecat_keeb_right
    ...
    artifact-name: alliecat_keeb_standard_right
  - board: alliecat_keeb_left
    shield: nice_view
    artifact-name: alliecat_keeb_standard_left
  ...
  - board: nice_nano_v2
    ...
    artifact-name: alliecat_keeb_dongle_central
...
create_peripheral_shields() {
    ...
    mkdir -p "$shields_dir/alliecat_keeb_peripheral"

    # Create peripheral overlay
    cat > "$shields_dir/alliecat_keeb_peripheral/alliecat_keeb_peripheral.overlay" << 'EOF'
...
EOF
    ...
    cat > "$shields_dir/alliecat_keeb_peripheral/alliecat_keeb_peripheral.conf" << 'EOF'
...
EOF
    ...
    cat > "$shields_dir/alliecat_keeb_peripheral/alliecat_keeb_peripheral.keymap" << 'EOF'
...
/*
 * Dongle Central Keymap - AllieCat Sofle
...
EOF
        ...
        grep -v "#include.*alliecat_keeb" "$PROJECT_ROOT/config/alliecat_keeb.keymap" | \
...
## Hardware Required
- 2x AllieCat Sofle keyboard halves
...
1. **Left Half**:
   ...
   - Flash `alliecat_keeb_dongle_peripheral_left.uf2`
   - Wait for restart

2. **Right Half**:
   - Enter bootloader mode (double-tap reset)
   - Flash `alliecat_keeb_dongle_peripheral_right.uf2`
   - Wait for restart
...
1. **Dongle (Nice!Nano with OLED)**:
   ...
   - Flash `alliecat_keeb_dongle_central.uf2`
   - Wait for restart
...
- Left: `alliecat_keeb_standard_left.uf2`
- Right: `alliecat_keeb_standard_right.uf2`
EOF
...
interactive_flashing_assistant() {
    ...
    local required_files=(
        "alliecat_keeb_dongle_peripheral_left.uf2"
        "alliecat_keeb_dongle_peripheral_right.uf2"
        "alliecat_keeb_dongle_central.uf2"
    )
    ...
    prompt_user "1. Put LEFT half in bootloader mode (double-tap reset button)\n2. Copy alliecat_keeb_dongle_peripheral_left.uf2 to the USB drive that appears\n3. Wait for device to restart"

    log_step "Flashing Right Half (Peripheral)"
    prompt_user "1. Put RIGHT half in bootloader mode (double-tap reset button)\n2. Copy alliecat_keeb_dongle_peripheral_right.uf2 to the USB drive that appears\n3. Wait for device to restart"

    log_step "Flashing Dongle (Central)"
    prompt_user "1. Put DONGLE in bootloader mode (double-tap reset button)\n2. Copy alliecat_keeb_dongle_central.uf2 to the USB drive that appears\n3. Wait for device to restart"
    ...
    cat > "$PROJECT_ROOT/.vscode/launch.json" << 'EOF'
{
    ...
    "configurations": [
        ...
        },
        {
            ...
            "type": "node-terminal",
            "request": "launch",
            "command": "west build --board alliecat_keeb_left --shield nice_view -- -DZMK_CONFIG=\"$(pwd)/config\" -DCONFIG_ZMK_STUDIO=y",
            "cwd": "${workspaceFolder}"
        }
    ]
}
...
# Main execution
main() {
    echo "🎮 ZMK AllieCat Sofle Dongle Configuration Setup"
    echo "================================================"
    echo ""
