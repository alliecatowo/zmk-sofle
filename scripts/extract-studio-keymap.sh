...
wait_for_keyboard() {
    log_step "Please connect your AllieCat Sofle keyboard via USB cable"
...
manual_extraction_guide() {
    ...
    echo "2. Connect to your AllieCat Sofle keyboard"
...
cp config/alliecat_keeb.keymap config/alliecat_keeb.keymap.backup

# If you have a .keymap file, replace the current one
if [ -f "config/my_studio_*.keymap" ]; then
    echo "Found extracted .keymap file"
    echo "Please manually review and merge with config/alliecat_keeb.keymap"
fi
...
- `config/alliecat_keeb.keymap` - Current repository keymap
...
2. **Compare with Current**
   ```bash
   diff config/alliecat_keeb.keymap config/my_studio_*.keymap
   ```
...
4. **Test Build**
   ```bash
   west build --board alliecat_keeb_left
   ```
