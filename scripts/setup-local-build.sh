...
echo "  Left:  west build -d build/left -s zmk/app -b alliecat_sofle_left -- -DZMK_CONFIG=\$(pwd)/config -DCONFIG_ZMK_STUDIO=y"
echo "  Right: west build -d build/right -s zmk/app -b alliecat_sofle_right -- -DZMK_CONFIG=\$(pwd)/config -DCONFIG_ZMK_STUDIO=y"
echo ""
echo "Dongle mode (central + 2 peripherals):"
echo "  Dongle: west build -d build/dongle -s zmk/app -b alliecat_sofle_dongle -- -DZMK_CONFIG=\$(pwd)/config -DCONFIG_ZMK_STUDIO=y -DCONFIG_ZMK_SPLIT_BLE_CENTRAL_PERIPHERALS=2"
echo "  Left:   west build -d build/left_periph -s zmk/app -b alliecat_sofle_left -- -DZMK_CONFIG=\$(pwd)/config -DCONFIG_ZMK_STUDIO=y -DCONFIG_ZMK_SPLIT_ROLE_CENTRAL=n"
echo "  Right:  west build -d build/right_periph -s zmk/app -b alliecat_sofle_right -- -DZMK_CONFIG=\$(pwd)/config -DCONFIG_ZMK_STUDIO=y -DCONFIG_ZMK_SPLIT_ROLE_CENTRAL=n"
echo ""
echo "Settings reset:"
echo "  west build -d build/reset -s zmk/app -b alliecat_sofle_left -- -DZMK_CONFIG=\$(pwd)/config -DSHIELD=settings_reset"
echo ""
